...
def get_mission(mid: int, db: Session = Depends(get_db)):
...
def update_mission(mid: int, payload: schemas.MissionUpdate, db: Session = Depends(get_db)):
    m = db.get(models.Mission, mid)
    if not m:
        raise HTTPException(404, "Mission not found")
...
def delete_mission(mid: int, db: Session = Depends(get_db)):
    m = db.get(models.Mission, mid)
    if not m:
        raise HTTPException(404, "Mission not found")
    db.delete(m)
...
def export_ics(range: str | None = None, db: Session = Depends(get_db)):
