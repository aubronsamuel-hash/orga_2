...
def list_availability(db: Session = Depends(get_db)):  # noqa: B008
    return db.query(models.Availability).all()


@router.post("", response_model=schemas.AvailabilityOut, status_code=201)
def create_availability(payload: schemas.AvailabilityCreate, db: Session = Depends(get_db)):  # noqa: B008
...
def conflicts(mission_id: int, db: Session = Depends(get_db)):  # noqa: B008
...
def get_availability(aid: int, db: Session = Depends(get_db)):  # noqa: B008
...
def update_availability(aid: int, payload: schemas.AvailabilityUpdate, db: Session = Depends(get_db)):  # noqa: B008
    a = db.get(models.Availability, aid)
    if not a:
        raise HTTPException(404, "Availability not found")
...
def delete_availability(aid: int, db: Session = Depends(get_db)):  # noqa: B008
    a = db.get(models.Availability, aid)
    if not a:
        raise HTTPException(404, "Availability not found")
    db.delete(a)
