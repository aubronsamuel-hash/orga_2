...
def list_technicians(db: Session = Depends(get_db)):  # noqa: B008
    return db.query(models.Technician).all()


@router.post("", response_model=schemas.TechnicianOut, status_code=201)
def create_technician(payload: schemas.TechnicianCreate, db: Session = Depends(get_db)):  # noqa: B008
...
def get_technician(tech_id: int, db: Session = Depends(get_db)):  # noqa: B008
...
def update_technician(tech_id: int, payload: schemas.TechnicianUpdate, db: Session = Depends(get_db)):  # noqa: B008
    t = db.get(models.Technician, tech_id)
    if not t:
        raise HTTPException(404, "Technician not found")
...
def delete_technician(tech_id: int, db: Session = Depends(get_db)):  # noqa: B008
    t = db.get(models.Technician, tech_id)
    if not t:
        raise HTTPException(404, "Technician not found")
    db.delete(t)
